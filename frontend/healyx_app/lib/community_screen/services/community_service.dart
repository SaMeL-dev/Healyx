...
class CommunityService {
  ...
  Future<http.Response> _retryJsonPutWithRefresh({
    required Uri uri,
    required String token,
    required Map<String, dynamic> body,
    Map<String, String>? extraHeaders,
    ...
    if (response.statusCode == 401) {
      debugPrint('[Community] json put accessToken expired. Trying refresh.');
  ...
  Future<void> updateComment({
    ...
    final response = await _retryJsonPutWithRefresh(
      uri: uri,
      token: token,
      body: {
        'content': content,
    ...
    debugPrint('[Community] updateComment status: ${response.statusCode}');
    debugPrint('[Community] updateComment body: $responseBody');
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      try {
        ...
        if (decoded is Map<String, dynamic> && decoded['success'] == false) {
          throw Exception(decoded['message'] ?? '댓글 수정에 실패했습니다.');
        }
      } on FormatException {
        return;
      }
